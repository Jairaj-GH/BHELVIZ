...
function Dashboard({ userRole, onLogout, notify, token }) {
  const USE_NEW_UI = typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.VITE_USE_NEW_UI ? (import.meta.env.VITE_USE_NEW_UI === 'true') : true;
    ...
    try {
      if (token) {
        const resp = await fetch('/query', {
          ...
          body: JSON.stringify({ utterance, session_id: `web-${Date.now()}`, history }),
        });
        ...
        display = rows.map(r => ({
          'Emp No': r.employee_no || r.employee_no_enc || r.employee_no || '',
          'Name': r.employee_name || r.full_name || r.full_name_enc || '',
          ...
          'Attendance': r.attendance_ts || null,
        ...
        setResults({ display, chart, rawCount, intent: data.intent || irRes.intent, desc: data.description || irRes.description });

        const botMsg = { role: 'assistant', type: 'structured', structured: { data: { display }, intent: data.intent || irRes.intent, description: data.description || irRes.description }, ts: new Date().toLocaleTimeString("en-IN") };
        ...
        const botMsg = { role: 'assistant', type: 'structured', structured: { data: { display }, intent: irRes.intent, description: irRes.description }, ts: new Date().toLocaleTimeString("en-IN") };
        setMessages(m => [...m, botMsg]);
      }
    ...
    <div className="fc-theme-app">
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column'}}>
        ...
        <div style={{ padding: 12, borderBottom: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 20, color: C.goldBright }}>BHELVIZ</div>
