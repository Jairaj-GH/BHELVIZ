...
class DepartmentOrm(Base):
    ...
    dept_name_enc = Column(String(512), nullable=False)
...
FIRST_NAMES = [
    "Arjun", "Priya", "Vikram", "Anita", "Ravi", "Sunita", "Mohan", "Lata", "Sanjay", "Deepa",
    "Kiran", "Rajesh", "Meena", "Suresh", "Kavya", "Anil", "Pooja", "Ramesh", "Seema", "Vinod",
]
LAST_NAMES = [
    "Sharma", "Kumar", "Patel", "Singh", "Nair", "Reddy", "Rao", "Mehta", "Joshi", "Iyer",
    "Gupta", "Verma", "Das", "Shah", "Pillai", "Bose", "Chatterjee", "Mishra", "Tiwari", "Dubey",
]


def seed_database(session) -> None:
    """Populate dev database with realistic mock employees + attendance."""
    import random
...
def get_dev_engine():
    ...
    if _dev_engine is None:
        ...
        with _dev_session_factory() as session:
            ...
            # Recreate views so they always match the current code.
            session.execute(text("DROP VIEW IF EXISTS employee_attendance_v"))
            session.execute(text("DROP VIEW IF EXISTS employee_leave_v"))

            # Dev SQLite views that mimic the Oracle-facing logical view shape.
            session.execute(text("""
                CREATE VIEW employee_attendance_v AS
                SELECT
                    a.attendance_id AS attendance_id,
                    e.employee_id AS emp_id,
                    e.full_name_enc AS full_name,
                    d.dept_code AS dept_name,
                    a.att_date AS att_date,
                    a.status_code AS status,
                    a.source_channel AS check_in,
                    a.source_channel AS check_out
                FROM attendance a
            ...
            session.execute(text("""
                CREATE VIEW employee_leave_v AS
                SELECT
                    1 AS request_id,
                    e.employee_id AS emp_id,
                    e.full_name_enc AS full_name,
...
def _oracle_env_present() -> bool:
    return all([DB_USER, DB_PASSWORD, DB_HOST, DB_PORT, DB_SERVICE])
...
def get_oracle_engine():
    ...
    if _oracle_engine is None:
        if not _oracle_env_present():
            raise RuntimeError(
                "Oracle environment is not fully configured. "
                "Set DB_USER, DB_PASSWORD, DB_HOST, DB_PORT, and DB_SERVICE."
            )
        ...
        try:
            ...
            msg = str(e)
            if "ORA-28365" in msg or "Wallet is not open" in msg:
                raise RuntimeError(
                    "Oracle Wallet appears to be closed (ORA-28365).\n"
                    "Ensure the database wallet is unlocked/opened for the DB user.\n"
                    "If you intended to run in dev mode, unset DB_USER/DB_PASSWORD so the\n"
                    "application uses the bundled SQLite dev DB instead.\n"
                    "See: https://docs.oracle.com/error-help/db/ora-28365/"
                ) from e
            raise
...
def get_session():
    """
    Return a DB session: Oracle if env vars are present, else dev SQLite.
    """
    if _oracle_env_present():
        return get_oracle_session()
