...
ORACLE_DSN    = os.environ.get("BHELVIZ_ORACLE_DSN", "")
...
async def login(form: OAuth2PasswordRequestForm = Depends(), request: Request = None):
    ...
    """
    # Demo credentials for testing — production: verify bcrypt hash + DB lookup
    if form.username == "admin@bhel.in" and form.password == "admin":
    ...
    if form.username.endswith("@bhel.in") and len(form.password) >= 4:
        token = create_access_token(user_id=999, email=form.username, role="user")
        _audit("LOGIN_SUCCESS", form.username, "User login", request=request)
        return {"access_token": token, "token_type": "bearer"}
